...
async function saveTranscript(messages) {
  try {
    const firstMsg = messages[0]?.content || 'unknown';
    const sessionId = Buffer.from(firstMsg).toString('base64').slice(0, 16);
    const transcript = messages
      .map(m => `${m.role === 'user' ? 'VISITOR' : 'MAYA'}: ${m.content}`)
      .join('\n\n');
    await fetch(TRANSCRIPT_WEBHOOK, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        last_visitor_message: [...messages].reverse().find(m => m.role === 'user')?.content || '',
        transcript: transcript
      })
    });
    console.log('Transcript saved, session:', sessionId);
  } catch (e) {
    console.error('Transcript save failed:', e.message);
  }
}
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (messages.length >= 2) saveTranscript(messages);
    if (messages.length >= 6) extractAndSend(messages, apiKey);
